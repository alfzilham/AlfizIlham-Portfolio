...
- **ChromaGrid Showcase** between the subtitle and the circular gallery (admin-curated via Editor Mode — see SPEC.md §14): 3×320px **white cards**, borderless with a **hard offset shadow** (`8px 8px 0 #0a0a0a`, growing to `12px` on hover) in the monochrome neo-brutalist style; per-card radial spotlight on hover; grid-level grayscale mask follows the cursor (grayscale only — no dimming); hover lifts the card (`translateY(-4px)`) above the mask revealing full color anywhere in the grid; no image zoom or border-color change on hover; kebab ⋮ menu visible only in editor mode; click opens the macOS-style lightbox; public view paginates 6 at a time with a "Load More Projects" outline button
- **Stats strip** below the grid: 3-column layout — featured-card count (left) · linking sentence + arrow-down icon toward the gallery (center) · circular-gallery works count (right); numbers are live from the database
