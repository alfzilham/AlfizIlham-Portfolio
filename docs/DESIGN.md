...
- **ChromaGrid Showcase** between the subtitle and the circular gallery (admin-curated via Editor Mode — see SPEC.md §14): 3×320px **white cards** (matching the page background) with rotating colored hover-border accents, per-card radial spotlight on hover, image scale ×1.06, grid-level grayscale mask following the cursor (grayscale only — no dimming); the hovered card lifts above the mask (`z-index: 5`) revealing full color anywhere in the grid; kebab ⋮ menu visible only in editor mode; click opens the macOS-style lightbox; public view paginates 6 at a time with a "Load More Projects" outline button
