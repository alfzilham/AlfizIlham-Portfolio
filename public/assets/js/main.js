...
/* --------------------------------------------------------------------------
   DEPTH CAROUSEL (vanilla port of ReactBits DepthCarousel — GSAP)
   -------------------------------------------------------------------------- */

function initDepthCarousel() {
  const root = document.getElementById("depthCarousel");
  ...
  const dotsWrap = root.querySelector(".depth-carousel__dots");
  ...
  function setActive(idx) {
    ...
    cards.forEach((c, i) => c.setAttribute("aria-hidden", i === idx ? "false" : "true"));
  }
  ...
  function layout(p) {
    ...
    for (let i = 0; i < count; i++) {
      ...
      el.style.transform = `translate(-50%, -50%) scale(${scale}) translateX(${tx.toFixed(
        2
  ...
  function tweenTo(target, animate) {
    if (tween) tween.kill();
    const dur = animate && !reduceMotion ? cfg.duration / 1000 : 0;
    const proxy = { p: pos };
    tween = gsap.to(proxy, {
      p: target,
      duration: dur,
  ...
  const navigateBy = (step) => setFocus(focusIdx + step, true);
  ...
  if (!reduceMotion && count > 1) {
    ...
    const start = () => {
      ...
      autoTimer = setInterval(() => {
        if (!hovered && !focusedEl) navigateBy(1);
